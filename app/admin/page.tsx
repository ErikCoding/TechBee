...
export default async function AdminPage() {
  ...
  const maxRole = Math.max(...stats.usersByRole.map((r) => r.count))
    ...
    <div className="flex flex-col gap-6">
          ...
          return (
            <div key={card.label} className="animate-fade-in-up rounded-2xl border border-border bg-card p-5" style={{ animationDelay: `${i * 60}ms` }}>
              <div className="flex items-center justify-between">
                ...
                {'trend' in card && (
                  card.trend ? <TrendingUp className="h-3.5 w-3.5 text-emerald-500" /> : <TrendingDown className="h-3.5 w-3.5 text-red-500" />
                )}
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <section className="rounded-2xl border border-border bg-card p-6 lg:col-span-2">
          <h2 className="font-semibold text-foreground">Przychód platformy</h2>
          <div className="mt-6 flex items-end gap-3" role="img" aria-label="Wykres słupkowy przychodu platformy">
                ...
                <div key={entry.month} className="flex flex-1 flex-col items-center gap-2">
                  ...
                  <div className="relative w-full rounded-t-lg bg-muted" style={{ height: '120px' }}>
                    <div
                      ...
                      style={{ height: `${heightPct}%`, background: entry.month === 'Lip' ? '#F4B400' : '#E4E4E7' }}
                    />
                  </div>
        ...
        <section className="rounded-2xl border border-border bg-card p-6">
          <h2 className="font-semibold text-foreground">Użytkownicy wg roli</h2>
